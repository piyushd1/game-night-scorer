...
## 2024-05-30 - [Unified array iterations for derived round data]
**Learning:** In `public/js/screens/dashboard.js`, multiple independent O(N) iterations over the `rounds` array were being performed during the synchronous render cycle to compute derived metadata (`roundPoints`, `roundFlip7Meta`, `roundJuaMeta`). This caused redundant loop overhead and unnecessary garbage collection.
**Action:** When optimizing synchronous rendering cycles that perform multiple separate O(N) or O(P*R) derived calculations, combine and merge these calculations into a single, unified cache memoization block to avoid redundant independent iterations.
