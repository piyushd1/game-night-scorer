...
## 2024-06-03 - [O(N) Array Allocation and Reduce in Hot Loops]
**Learning:** Found that `_computeFlip7Score` in `dashboard.js` was converting `Set` objects into arrays using `[...set]` and calling `Array.reduce` to calculate sums on every draft entry change in the Flip7 scoring drawer. Because this function runs in high-frequency paths (e.g., inside `_renderDrawer` on any UI interaction), these intermediate array allocations and functional callbacks generated measurable GC pressure and overhead.
**Action:** In hot UI rendering paths or continuous recalculation loops, iterate over iterables directly with `for...of` loops rather than using spreading (`[...iterable]`) and declarative functional methods like `.reduce()` that allocate temporary arrays.
