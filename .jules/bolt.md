...
## 2024-07-14 - [O(N) Intermediate Array Allocations in Iterations]
**Learning:** Using chained array methods (like `Object.entries().map()` or `Array.filter().length`) inside hot loops or UI render methods causes excessive, redundant O(N) intermediate array allocations and GC pauses. In `stats.js`, computing `minCardTotal` per round dynamically allocated multiple arrays. In `recap.js`, `.filter()` was used per-player on every render to compute medal counts.
**Action:** Always prefer direct `for...of` loops over `Object.values()` when computing min/max to avoid array allocations, and proactively precompute metrics like medal counts onto the data objects (e.g. `ones`, `twos`) during the state aggregation phase so UI render components can use O(1) property access instead of O(N) array filtering.
