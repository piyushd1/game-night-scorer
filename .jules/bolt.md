...
## 2024-05-30 - [Separated O(P*R) rendering loops]
**Learning:** Found that `roundJuaMeta` in `public/js/screens/dashboard.js` was being computed using an un-memoized `.map()` over rounds for each player (O(P*R)), immediately following an existing `WeakMap` cached loop that iterated over the exact same rounds and players for `roundPoints`. This resulted in `P` redundant array allocations and unnecessary O(P*R) operations on every dashboard render.
