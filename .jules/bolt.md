...
## 2026-06-04 - [Memoizing Jua Meta in Dashboard]
**Learning:** Found that `roundJuaMeta` calculation in `public/js/screens/dashboard.js` was performing an O(P*R) operation on every render, separate from the already memoized `roundPoints` calculation. This caused redundant array allocations and iterations during frequent UI render cycles.
**Action:** When optimizing synchronous rendering cycles that perform multiple separate O(P*R) derived calculations (e.g., mapping game rounds for multiple metadata types), combine and merge these calculations into a single, unified cache memoization block to avoid redundant independent iterations.
