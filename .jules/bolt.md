...
## 2024-05-04 - [Dashboard O(R*P) Recalculation on Render]
**Learning:** Dashboard logic iterates O(R*P) nested loops to calculate `roundPoints` on every single render. Because Firebase sync triggers multiple state updates simultaneously (for meta, players, games), it effectively forces multiple immediate redundant recalculations of the exact same unchanged rounds on every sync.
**Action:** Always memoize derived round calculations using a `WeakMap` bound directly to the `rounds` Firebase state object reference, bypassing synchronous multi-event UI re-renders.
