...
## 2024-05-15 - [Dashboard Render O(R*P) Recalculation]
**Learning:** Found that `roundPoints` calculation in `public/js/screens/dashboard.js` recomputes `gameModule.getRoundPoints` for every round and player on every single dashboard re-render (which happens on any Firebase update). Since `game.rounds` object reference is replaced completely by Firebase state syncing when changed, it can be cached cleanly using a `WeakMap`.
**Action:** Use `WeakMap` keyed by `game.rounds` object to memoize `roundPoints` calculations. Also ensure the caching structure validates dependent fields (e.g., `playerIdsRef === playerIds`) to prevent stale data if other dependent array lengths change independently.
