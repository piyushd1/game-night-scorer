...
## 2024-05-30 - [O(P*R) Bottleneck in Dashboard Meta Calculation]
**Learning:** Found that `roundJuaMeta` computation iterated over all players and rounds independently of `roundPoints`, leading to redundant O(P*R) calculations and missing out on the `_roundPointsCache`.
**Action:** Combine and merge multiple separate O(N) or O(P*R) derived calculations (e.g., mapping game rounds for multiple metadata types) into a single, unified cache memoization block to avoid redundant independent iterations.
