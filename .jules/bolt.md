...
## 2024-05-30 - [Redundant mapping of Jua metadata in Dashboard]
**Learning:** Found that `roundJuaMeta` in `public/js/screens/dashboard.js` was being computed on every render outside of the `_roundPointsCache` block, causing redundant O(P*R) operations even when the core round data was successfully retrieved from the WeakMap cache.
**Action:** When optimizing synchronous rendering cycles that perform multiple separate O(N) or O(P*R) derived calculations (e.g., mapping game rounds for multiple metadata types), always combine and merge these calculations into a single, unified cache memoization block to avoid redundant independent iterations.
