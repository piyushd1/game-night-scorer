...
function _render(container, roomCode) {
  ...
  if (!cacheHit) {
    ...
    // Bolt Optimization: Unified cache memoization block.
    // Merged multiple O(P*R) derived calculations (points, flip7Meta, juaMeta) into a single iteration over rounds.
    rounds.forEach((rnd) => {
      playerIds.forEach((pid) => {
