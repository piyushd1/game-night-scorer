...
function _render(container, roomCode) {
  ...
  if (!cacheHit) {
    ...
    // Bolt Optimization: Computed derived round meta values like `roundJuaMeta`
    // (per-player jua first-save metadata) inside the main cached rounds loop.
    // This turns multiple separate O(P*R) rendering operations into a single loop
    // that is cleanly memoized across multiple renders.
    rounds.forEach((rnd) => {
      playerIds.forEach((pid) => {
    ...
    if (game.rounds && typeof game.rounds === 'object') {
      _roundPointsCache.set(game.rounds, { result: roundPoints, flip7Meta: roundFlip7Meta, juaMeta: roundJuaMeta, playerIds });
    }
  }
