...
function _renderSelection(container) {
  container.querySelectorAll('.game-card').forEach((card) => {
    ...
    if (isSelected) {
      ...
      indicator.innerHTML = '<span class="material-symbols-outlined text-white text-base" aria-hidden="true">check</span>';
    } else {
  ...
  if (_selectedGame) {
    ...
    startBtn.innerHTML = `START ${game.label.toUpperCase()} <span class="material-symbols-outlined text-lg" aria-hidden="true">arrow_forward</span>`;
  } else {
...
async function _startGame(container, roomCode) {
  ...
  try {
    ...
    btn.disabled = false;
    btn.innerHTML = 'START GAME <span class="material-symbols-outlined text-lg" aria-hidden="true">arrow_forward</span>';
  }
}
