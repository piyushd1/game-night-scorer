...
function _render(container, roomCode) {
  ...
  if (activePlayerIds.length === 0) {
    container.innerHTML = `
      <div class="p-6 text-center py-20">
        <span class="material-symbols-outlined text-5xl text-outline mb-4" aria-hidden="true">person_off</span>
  ...
  if (game.type === 'papayoo') {
    ...
    if (rounds.length >= limit && game.status === 'active') {
      container.innerHTML = `
        <div class="p-6 text-center py-20">
          <span class="material-symbols-outlined text-5xl text-outline mb-4" aria-hidden="true">check_circle</span>
...
async function _submitRound(container, roomCode, initialGame, gameModule) {
  ...
  try {
    ...
    btn.disabled = false;
    btn.innerHTML = 'CONFIRM ROUND <span class="material-symbols-outlined text-lg" aria-hidden="true">check</span>';
  }
}
